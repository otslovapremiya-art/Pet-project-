...
|---|---|---|---|---|---|
| 1 | Как проверить, что код делает заявленное, если я его не читаю? | Без ответа стадия «тестирование» в цикле — фикция, и схема будет отлажена неправильно | На первой же функции заранее записать в задачу три проверки, выполняемые руками в браузере: жму — растёт на 1; жму десять раз — ровно десять; перезагружаю — начинается заново. Провести их. Один вечер | | открыт |
| 2 | Хватит ли `STATUS.md`, чтобы новая сессия продолжила с нужного места? | Это главная проверяемая гипотеза всей схемы: если контекст не передаётся, схема не работает | После первой готовой функции закрыть сессию, начать новую с нуля, дать только `CLAUDE.md` и `STATUS.md` и фразу «продолжай». Считается пройденным, если новая сессия сама назвала следующий шаг и не переспрашивала про прошлое. 30 минут | | открыт |
...
| ~~Claude может выполнять git-команды на машине через мост~~ | **Проверено 09.08.2026 — допущение неверно.** Сессия работает в облаке, доступа к диску нет. Решение: git стоит у Vitalii, Claude выдаёт готовые команды строкой, Vitalii копирует их в терминал | Закрыто |
| Файлы переносятся между облаком и диском вручную и ничего не теряется | Работа предыдущей сессии пропадёт вместе с облачным контейнером | Ритуал конца сессии в 5.4 |
...
| Vitalii сможет сформулировать проверки функции своими словами | Приёмка станет формальной, схема отладится неправильно | Вопрос №1 в 1.3 |
...
- [x] **1. Проверить мост.** ~~Убедиться, что Claude может выполнять команды на машине Vitalii.~~ **Проверено 09.08.2026: моста нет.** Сессия идёт в облаке. Режим работы: Claude собирает файлы у себя и отдаёт архивом, Vitalii распаковывает в папку проекта; команды в терминале Vitalii выполняет сам по готовым строкам.
- [ ] **2. Установить git.** Скачать и установить, проверить, что команда `git` отвечает. Настроить имя и почту. *Выполняет Vitalii по инструкции.*
- [ ] **3. Создать аккаунт GitHub** *(делает Vitalii лично — пароли и регистрация)*. Создать **публичный** репозиторий, связать с локальной папкой.
- [ ] **4. Сделать первый коммит и один тестовый откат.** Не для галочки: сломать что-нибудь нарочно и вернуть обратно, чтобы страховка была проверена, а не только настроена.
- [ ] **5. `.gitignore`.** Минимальный: системный мусор Windows. `.env` в проекте нет и не будет — см. 3.4.
- [ ] **6. `CLAUDE.md`.** Содержит: чем является проект (3 строки), как запустить игру, договорённости по стилю кода (**«пиши так, чтобы новичок прочёл»**), что нельзя трогать, ссылки на остальные документы. **Не** содержит пересказ архитектуры и историю изменений — для них есть отдельные файлы.
- [ ] **7. `ARCHITECTURE.md`** — раздел 3.2 этого документа, дальше живёт своей жизнью.
- [ ] **8. `STATUS.md`** — где мы сейчас, чем закончили прошлую сессию, что следующим шагом. Главный файл схемы: именно он проверяется вопросом №2.
- [ ] **9. `CHANGELOG.md`** — журнал изменений, по записи на функцию. Он же доказательство критерия успеха.
- [ ] **10. Шаблон задачи.** Три поля: что не так / как должно быть / **как проверить руками**. Третье поле обязательно — это ответ на вопрос №1.
...
| 09.08.2026 | Работаем в облаке, файлы переносим вручную | Перезапустить задачу на компьютере | Выбор Vitalii. Цена: ручной перенос файлов в конце каждой сессии |
| 09.08.2026 | git ставится у Vitalii, команды он копирует строкой | git в облаке с архивированием истории; отложить git | История коммитов должна пережить перерыв между сессиями, а облачный контейнер — не переживает. Побочный плюс: Vitalii видит, что делает git |
