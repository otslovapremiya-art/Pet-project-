...
| 3 | Окупаются ли задачи, ветки и четыре документа на проекте из трёх файлов? | Если ритуал тяжелее проекта, схему надо иметь в двух размерах — лёгком и полном | На первых трёх функциях засекать время отдельно на код и отдельно на документы с задачами. Если бумага съедает больше половины — схему для мелких проектов упрощаем. Отдельного эксперимента не требует, замер идёт по ходу | **Да, окупается.** Функция 1 — около 3 часов времени владельца, функции 2 и 3 вместе — около часа, то есть примерно полчаса на функцию, и в тот же час уместились правка схемы и разбор развилки с MVP. Решение 10.08.2026: облегчённую версию схемы не делаем, работаем как есть. Оговорки: оценка грубая, названа по памяти; и само деление «код против бумаги» оказалось негодным — владелец код не пишет и не читает, поэтому вся его цена и есть разговоры с документами | закрыт |
...
| 8 | **Вынести схему из игры** — см. 5.5 | нет | Комплект перенесён в пустую папку и не содержит ни слова про счётчик, тик и улучшения |
...
### 5.5 Вынести схему из игры — шаг 8

> Записано 10.08.2026 по прямой просьбе Vitalii: «нужно сделать запись, которая напомнит мне об этом после MVP».

**Зачем этот шаг существует.** Главный результат проекта — не игра, а схема. Но сегодня схема не отделена от игры: она живёт внутри файлов, набитых фактами про счётчик, тик и цену улучшения. Чтобы начать по ней второй проект, придётся копировать эти файлы и вычищать игру руками — а это ровно то место, где человек без опыта спотыкается и бросает.

Между «сделать пять функций» и «развернуть второй проект» в плане была дыра. Шаг 8 её закрывает.

**Что надо сделать.**

1. Пройти по шести документам и разделить в каждом две вещи: что здесь **правило схемы**, годное для любого проекта, и что здесь **факт про эту игру**. Например, «состояние игры хранится в одном объекте» — факт; «архитектурные решения записываются до кода, вместе с причиной отказа от альтернатив» — правило.
2. Собрать правила в чистый комплект шаблонов: пустые `PROJECT.md`, `CLAUDE.md`, `ARCHITECTURE.md`, `STATUS.md`, `CHANGELOG.md`, `TASK-TEMPLATE.md` — с пояснениями, зачем каждый файл и что в него писать, но без единого слова про игру.
3. Отдельно записать то, что схемой стало по итогам ошибок, а не по замыслу. На 10.08.2026 это: пять шагов заведения задачи; правило «Claude называет непокрытые категории, но не формулирует пункты за владельца»; правило «истина живёт на диске»; отказ от автотестов в пользу приёмки руками.
4. **Проверить перенос на пустой папке.** Скопировать комплект, начать по нему разговор с нуля и посмотреть, доходит ли дело до первой функции без подсказок. Это и есть критерий успеха из части 1.1 — значит шаг 8 не подготовка к проверке, а сама проверка.

**Чем это может оказаться, кроме папки с файлами.** У Vitalii уже установлен скилл `zapusk-proekta` — именно он провёл интервью, из которого вырос этот документ. То есть половина схемы, запуск, уже существует в переносимом виде. Схема **ведения** — цикл функции, шаблон задачи, правила приёмки — пока нигде, кроме этих файлов, не живёт. Логичный вид для неё — такой же скилл, чтобы новый проект подхватывал правила сам, а не через копирование папки. Решать в момент шага 8, не сейчас.

**Чего на этом шаге не делать.** Не улучшать схему по ходу переноса. Переносим то, что проверено на этом проекте, и ничего сверх: непроверенное правило в чистом комплекте выглядит так же убедительно, как проверенное, и отличить их потом будет нельзя.

### 5.6 Определение готовности
